...
parser.add_argument('--resume', '-r', default='sysu_agw_p8_n4_lr_0.05_seed_0_best.t', type=str, help='resume from checkpoint')
...
if dataset == 'sysu':
    ...
    for trial in range(10):
        test_trial = trial +1
        #model_path = checkpoint_path +  args.resume
        model_path = checkpoint_path + 'regdb_agw_p8_n4_lr_0.1_seed_0_trial_1_best.t'.format(test_trial)
        if os.path.isfile(model_path):
            print('==> loading checkpoint {}'.format(args.resume))
            checkpoint = torch.load(model_path)
            net.load_state_dict(checkpoint['net'])
